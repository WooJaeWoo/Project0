...
var Data = {
	...
	getData: function(col) {
		$.ajax({
			...
			success: function(result) {
				this.drawChart(this.refineResult(result));
			}.bind(this),
	...
	refineResult: function(result) {
		...
		result.forEach(function(element) {
			rResult[element._id] = element.count;
		});
		
		return rResult;
	},
	drawChart: function(result) {
		var svg = d3.select("main").append("svg").append("g")
		...
		/* Result 가공하기 */
		var colorArr = ["#98abc5", "#8a89a6", "#7b6888", "#6b486b", "#a05d56", "#d0743c", "#ff8c00"];
		var domainArr = Object.keys(result);
		
		var color = d3.scale.ordinal()
		.domain(domainArr)
		.range(colorArr.slice(0, Object.keys(result).length));
